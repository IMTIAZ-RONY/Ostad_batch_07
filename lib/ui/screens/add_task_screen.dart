    ...
    _projectFocusNode.addListener(() {
      if (_projectFocusNode.hasFocus) {
        onProjectChanged(
          ...
          savedProjects,
              (List<String> projects) => setState(() => filteredProjects = projects),
        ...
        body: GestureDetector(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text(privateToYou, style: TextStyle(color:greyColor, fontSize: 14)),
                const SizedBox(height: 10),
                TextFormField(
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    const Spacer(),
                    DueDateWidget(startDate: startDate, startTime: startTime, endDate: endDate, endTime: endTime, onTap: _selectDateTime)
                  ],
                ),
                const SizedBox(height: 20),
                buildAddProject(
                  ...
                  onCreateProject: () {
                    final newProject = _projectController.text.trim();
                    if (newProject.isNotEmpty && !savedProjects.contains(newProject)) {
                  ...
                  onProjectChanged: (input ) {
                    onProjectChanged(input, savedProjects,
                        (List<String>projects)=>setState(() =>filteredProjects=projects),
                          (bool value) => setState(() => showNoProjectMessage = value),
                          (bool value) => setState(() => showProjectSuggestions = value),);
                  },
                ...
                if (shouldShowAddSubtask(controllers) && controllers.isEmpty)
                  TextButton(
                    onPressed: _addSubtaskField,
                    child: const Text(addSubtask, style: TextStyle(color: greyColor)),
                  ),
                const SizedBox(height: 20),
                ...
                buildBottomRow(
                  ...
                  onAttachDocument: () => attachDocument(attachFile: _attachFile),
